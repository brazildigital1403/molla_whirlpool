...
- [ ] Editar `public/assets/header.css` — variáveis CSS no topo:
  ```css
  :root {
    --mlh-navy: #003B5C;   /* cor primária do cliente */
    --mlh-blue: #2DB5DF;
    --mlh-green: #50E596;
    --mlh-teal: #27C7BD;
    --mlh-light: #EEF6F8;
...
Por enquanto o `localStorage` usa chaves `metlife_auth`,
`metlife_role`, `metlife_user`. Se quiser renomear pro nome do
novo cliente:
...
- [ ] Ajustar `window.MetLifeAuth` → `window.<Cliente>Auth` ou só `window.Auth`
...
### Limpar referências MetLife em comentários
- [ ] `grep -rl 'MetLife' public/` — ver onde aparece
- [ ] Substituir em comentários (não em conteúdo real do cliente exemplo)

### Renomear assets
- [ ] (Opcional) Renomear classes `mlh-*` no header.css/.js pra `<cliente>-*`
