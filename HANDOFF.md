...
> Template originado do projeto **Whirlpool Brasil** (Agência Molla).
...
12. [Páginas do Whirlpool · base vs específico](#12--páginas-do-whirlpool--base-vs-específico)
13. [Lições aprendidas do Whirlpool (26 sessões)](#13--lições-aprendidas-do-whirlpool-26-sessões)
...
|--------|------|----------|
| **Cliente** (role `whirlpool` no Whirlpool) | `whirlpool2026` | Visualizar tudo, comentar e aprovar criativos |
...
├── REFERENCIA_metlife_README.md         ← README original do projeto Whirlpool
├── package.json                         ← marker do Node (NÃO tem dependências)
├── vercel.json                          ← config de rotas (URLs limpas)
├── .gitignore
│
├── docs/
│   ├── REFERENCIA_metlife_MASTER.md     ← MASTER vivo do Whirlpool
...
└── public/                              ← TUDO QUE A VERCEL PUBLICA
    ...
    │
    ├── cronograma.html                  ← Exemplo: crono ads 75 dias (Whirlpool)
    ├── plano-midia.html                 ← Exemplo: plano estratégico (Whirlpool)
    ├── performance.html                 ← Exemplo: report semanal (Whirlpool)
    ├── muito-alem-do-jogo.html          ← Exemplo: programa institucional (Whirlpool)
    ├── blitz.html + blitz/              ← Exemplo: watch party + brindes (Whirlpool)
    ├── elemidia.html + elemidia/        ← Exemplo: mídia OOH em prédios (Whirlpool)
    │
    ...
    │   └── logo_whirlpool.webp             ← logo cliente exemplo
    │
...
Páginas inteiras criadas pra Whirlpool — funcionam, mas o conteúdo é
do projeto Whirlpool. Use de:
...
Páginas exemplo:
- `cronograma.html` — timeline diarizado com fases (75 dias Whirlpool)
...
# Não obrigatório, mas ajuda a não confundir com Whirlpool
```
...
```css
:root {
  --whp-navy: #0D436B;   /* TROCAR pela cor primária do cliente */
  --whp-blue: #00A0DD;   /* TROCAR pela cor secundária */
  --whp-green: #50E596;  /* opcional */
}
```
...
**c. Logo**: substitua `public/img/logo_whirlpool.webp` pelo logo do
...
- [ ] Páginas exemplo carregam sem erro (mesmo com conteúdo Whirlpool)
...
### Paleta (exemplo do Whirlpool — troque pelas cores do cliente)

| Token | Hex | Uso |
|-------|-----|-----|
| `--navy` | `#0D436B` | Cor primária |
| `--blue` | `#00A0DD` | Accent (hover, secundário) |
| `--teal` | `#00A0DD` | Verde-azulado (cards) |
| `--green` | `#50E596` | Positivo (sucesso, KPI verde) |
| `--light` | `#F1F5F9` | Background neutro |
...
section[id] { scroll-margin-top: calc(var(--whp-header-h, 60px) + 64px); }
...
- **Box-shadow padrão**: `0 12px 32px rgba(13, 67, 107, 0.08)`
...
- Login bem-sucedido grava `whirlpool_auth`, `whirlpool_role`, `whirlpool_user`
...
### API pública
```js
WhirlpoolAuth.login(password, userName)  // retorna boolean
WhirlpoolAuth.logout()                    // redireciona pra /login
WhirlpoolAuth.isAuthenticated()           // boolean
WhirlpoolAuth.getRole()                   // 'cliente' | 'molla' (renomear pro cliente novo)
WhirlpoolAuth.isAdmin()                   // boolean (true se role === 'molla')
WhirlpoolAuth.getUserName()               // string
WhirlpoolAuth.setUserName(name)
WhirlpoolAuth.ensureUserName()            // pergunta via prompt se faltar
WhirlpoolAuth.guard()                     // auto-roda em toda página
```

⚠️ Adapte o objeto global `window.WhirlpoolAuth` pro nome do novo
...
- 🔓 Bypass trivial: `localStorage.setItem('whirlpool_auth', '1')` no console
...
Lista prática de tudo que precisa trocar pra "tirar o sticker Whirlpool":
...
- [ ] `public/assets/auth.js` — `window.WhirlpoolAuth` → renomear se quiser
...
- [ ] Search & replace `Whirlpool` → `<Cliente>` em comentários do JS/CSS
...
## 12 · Páginas do Whirlpool · base vs específico
...
- 🟠 **ESPECÍFICO** — caso de uso muito atrelado ao Whirlpool/Copa

---

## 13 · Lições aprendidas do Whirlpool (26 sessões)

Resumo do que evoluiu ao longo do projeto Whirlpool (consulte
...
- 🔑 Senha admin atual do Whirlpool: `molla@2026@` (renove pro cliente novo)
...
| **`whp-*`** | Prefixo do header global ("Whirlpool Header" — pode renomear) |
